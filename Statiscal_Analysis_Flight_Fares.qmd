...
```{r}
unique(train$Journey_Day)


# Count of flights with different dates
ggplot(train, aes(x = Journey_Day)) +
  geom_bar(colour = "black", fill = "blue") +
  labs(title = 'Flight Count vs Date',
       x = 'Journey_Day',
       y = 'Count of flights') +
...
ggplot(train, aes(x = Journey_Day, y = Price)) +
  geom_bar(stat = 'summary', fun = 'mean', fill = 'steelblue') +
  labs(title = 'Price of flights with different dates',
       x = 'Date',
       y = 'Price') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
